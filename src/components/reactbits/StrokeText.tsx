...
const StrokeText: React.FC<StrokeTextProps> = ({
  ...
  const numFontSize = typeof fontSize === 'number' ? fontSize : parseFloat(String(fontSize)) || 128;
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, numFontSize * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  }, [characters, fontSize, fontWeight, letterSpacing, strokeWidth, numFontSize]);
  ...
  const viewBox = box
    ...
    : `0 ${-numFontSize * 0.9} ${characters.length * (numFontSize * 0.65)} ${numFontSize * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${Math.round(numFontSize * 1.3)}px` }}
      role="img"
